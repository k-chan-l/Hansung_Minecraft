...
여러 플러그인이 서로 이벤트를 주고받기 때문에,  
다른 플러그인의 이벤트나 클래스를 사용하는 경우 `pom.xml` 또는 라이브러리 설정에 의존성을 등록해야 합니다.

기존 README에서도 `MqttLungrow`, `NPCPlugin`, `httpconnection` 등은  
플러그인 간 이벤트 연동을 위해 라이브러리 등록이 필요하다고 명시되어 있습니다.

---

### 2. MQTT 접속 정보 관리

현재 코드에는 MQTT Broker 접속 정보가 직접 작성되어 있습니다.

실제 운영 또는 공개 프로젝트에서는 다음과 같이 분리하는 것이 좋습니다.
...
HTTP Endpoint 또는 DB 관련 정보도 코드에 직접 넣기보다  
설정 파일로 분리하는 것이 유지보수와 보안 측면에서 좋습니다.
